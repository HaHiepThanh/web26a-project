...
import { withEntities, upsertEntity, removeEntity, setAllEntities } from '@ngrx/signals/entities';
...
export const WorkspaceStore = signalStore(
  ...
  withComputed(({ entities }) => ({
    workspaces: computed(() => [...entities()].sort((a, b) => a.name.localeCompare(b.name))),
  })),

  withMethods((store, api = inject(ApiService)) => ({
    // Nhận từ WebSocket — LUÔN upsert, không bao giờ add.
    applyRemote(row: ApiWorkspace): void {
      patchState(store, upsertEntity(toWorkspace(row)));
    },

    async load(orgId: string): Promise<void> {
      patchState(store, { loading: true, error: null });
      try {
        const rows = await api.get<ApiWorkspace[]>(`/workspaces?orgId=${orgId}`);
        patchState(store, setAllEntities(rows.map(toWorkspace)), { loading: false });
      } catch (e) {
        patchState(store, { loading: false, error: describeError(e, 'Không tải được workspace.') });
      }
    },
  })),

  withHooks({
    onInit(store, rt = inject(RealtimeService)) {
      rt.on<ApiWorkspace>('workspace.updated', (d) => store.applyRemote(d));
    },
  }),
);
```
...
| `store/board*.store.ts`, `store/list*`, `store/chat*` | Hoà |
