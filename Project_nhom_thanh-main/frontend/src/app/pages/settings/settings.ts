...
function computePasswordStrength(password: string): PasswordStrength {
  if (!password) return { score: 0, label: '', percent: 0, colorVar: 'var(--color-base-300)' };
  ...
  const table: Record<number, { label: string; colorVar: string }> = {
    0: { label: 'Rất yếu', colorVar: 'var(--color-error)' },
    1: { label: 'Yếu', colorVar: 'var(--color-error)' },
    2: { label: 'Trung bình', colorVar: 'var(--color-warning)' },
    3: { label: 'Mạnh', colorVar: 'var(--color-success)' },
    4: { label: 'Rất mạnh', colorVar: 'var(--color-success)' },
  };
